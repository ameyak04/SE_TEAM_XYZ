...
//                dates.add("       Date          " + "Attendance");
                ...
                dbAttendance.addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(DataSnapshot dataSnapshot) {
                        for (DataSnapshot dsp : dataSnapshot.getChildren()) {
                            Log.d("dsp", String.valueOf(dsp));
                            Log.d("class",classname);
                            Log.d("sid",student_id);
                            Log.d("tid",teacher_id);

//                            p1 = dsp.child(classname).child(teacher_id).child(student_id).child("p1").getValue().toString();

                            if (dsp.hasChild(classname) &&
                                    dsp.child(classname).hasChild(teacher_id) &&
                                    dsp.child(classname).child(teacher_id).hasChild(student_id) &&
                                    dsp.child(classname).child(teacher_id).child(student_id).hasChild("p1")) {
                                p1 = dsp.child(classname).child(teacher_id).child(student_id).child("p1").getValue().toString();
                            } else {
